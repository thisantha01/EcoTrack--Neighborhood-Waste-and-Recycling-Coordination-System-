...
class ManagerRouteMapScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final stops =
        (route['routeStops'] as List?) ?? (route['stops'] as List?) ?? const [];
    final markers = <Marker>[
      Marker(
        point: center,
        width: 42,
        height: 42,
        child: const Icon(Icons.alt_route, color: Color(0xFF0097A7), size: 36),
      ),
    ];
    for (final stop in stops) {
      if (stop is Map &&
          stop['location'] is Map &&
          stop['location']['lat'] is num &&
          stop['location']['lng'] is num) {
        markers.add(
          Marker(
            point: LatLng(
              (stop['location']['lat'] as num).toDouble(),
              (stop['location']['lng'] as num).toDouble(),
            ),
            width: 36,
            height: 36,
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(route['routeName']?.toString() ?? 'Route Map'),
