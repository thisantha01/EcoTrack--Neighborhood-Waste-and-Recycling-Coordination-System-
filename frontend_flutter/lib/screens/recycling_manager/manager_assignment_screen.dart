...
class _ManagerAssignmentScreenState extends State<ManagerAssignmentScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (widget.request.lat != null && widget.request.lng != null) {
        provider
            .suggestRoute(lat: widget.request.lat!, lng: widget.request.lng!)
            .then((route) {
  ...
  Future<void> _confirm() async {
    ...
    if (operatingDays.isNotEmpty &&
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('This route operates on ${operatingDays.join(', ')}'),
        ),
      );
      return;
    }
      ...
      body: Consumer<ManagerProvider>(
        builder: (context, provider, child) {
          final matchingRoutes = provider.routes
              ...
              .toList();
          final selectedRoute = matchingRoutes.isEmpty
              ? null
          ...
          return ListView(
            ...
            children: [
              ...
              if (selectedRoute != null) ...[
                ...
                Text(
                  '${selectedRoute['zone'] ?? ''} • ${((selectedRoute['routeStops'] as List?) ?? (selectedRoute['stops'] as List?) ?? const []).length} current stops',
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          Text(
            '${_suggestedRoute?['routeName'] ?? 'Route'} (${_suggestedRoute?['zone'] ?? ''})',
            style: const TextStyle(fontWeight: FontWeight.bold),
          ),
          Text(
            'Permanent driver: ${driver is Map ? driver['name'] ?? 'Unassigned' : 'Unassigned'}',
          ),
          const SizedBox(height: 4),
          const Text(
            'Review this suggestion and change it before confirming if needed.',
